...
* 回答: {人物} 對 {問題} 的答案
  ...
  * 屬性
    ...
    * 受訪者: 回答問題的 {人物}
      * 受訪身份: {人物} 在回答此問題當下的身份
    * 受訪狀態: 親自作答 / 轉述作答 / 尚未回答 / 拒絕作答
      * ? 是否要把「拒絕作答」拿掉？因為如果要標記為拒絕回答還是得有引用來源才能證實他真的有拒絕，但一旦轉述了就應該歸為「轉述作答」才對，讓讀者自己去針對這個轉述內容作評價，而不是由系統或經營者來人工判斷對方是不是拒絕作答。而且沒有人會自己跑來說他拒絕作答，大多是相應不理…
    * 轉述者: 將 {人物} 在其他管道的回答內容轉述到系統上的 {用戶}
      * // 當受訪狀態是轉述作答時，這個欄位是必要的，反之則是空的
    * 轉述來源: 轉述內容的來源網址
      * // 當受訪狀態是轉述作答時，這個欄位是必要的，反之則是空的
  * 統計
    * 評分者: 針對這則回答給分的 {用戶} 們，每個 {用戶} 的給分內容包括
      * 得分:  {用戶} 對回答內容完整度的評價，從 1 顆星到 5 顆星
        * 1 顆星：
        * 2 顆星：
        * 3 顆星：
        * 4 顆星：
        * 5 顆星：
      * 支持度: {用戶} 對回答內容的支持度，從 1 顆星到 5 顆星
        * 1 顆星：
        * 2 顆星：
        * 3 顆星：
        * 4 顆星：
        * 5 顆星：
    * 平均得分: 所有 {用戶} 對回答內容完整度的評價
    * 平均支持度: 所有 {用戶} 對回答內容的支持度
